...
DEFAULT_BENCHMARK_IDS = ("B2S1", "B2S2", "B2S3", "B2S4", "B2S5", "B2S6")
...
CASE_SCALE_SPECS: Tuple[Tuple[int, int, str, str | None, Tuple[int, ...] | None], ...] = (
    (1, 1, "6-node, 18-example branch-pressure hidden-memory scale anchor.", "cvt1_task_a_stage1", None),
    (2, 2, "10-node, 36-example hidden-memory scale point on the large topology.", "cvt1_task_a_large", None),
    (3, 6, "30-node, 108-example hidden-memory scale point aligned with the original B2 scale setting.", "cvt1_task_a_scale", None),
    (4, 12, "50-node, 216-example hidden-memory scale point on the ceiling topology.", "cvt1_task_a_ceiling", None),
    (5, 24, "75-node, 432-example aspirational hidden-memory scale point on a deeper layered DAG.", None, (4, 6, 9, 12, 12, 11, 8, 6, 4, 2)),
    (6, 48, "100-node, 864-example aspirational hidden-memory scale point on a broader layered DAG.", None, (4, 8, 12, 16, 18, 16, 12, 8, 5)),
)


@dataclass(frozen=True)
class BScaleTaskSpec:
    task_key: str
...
def _hidden_state(history: Sequence[Sequence[int]], window: int) -> int:
    return _parity(_flatten_history(history, window))


def _scaled_b_task_signals(task_key: str, *, pass_count: int, memory_window: int) -> tuple[SignalSpec, ...]:
    ...
    for pass_index in range(pass_count):
        ...
        for value in BASE_VALUES:
            ...
            context_bit = _hidden_state(history, memory_window)
            transform = TASK_TRANSFORMS[task_key][context_bit]
            signals.append(
                SignalSpec(
                    input_bits=bits,
...
def _registered_topology_case(
    *,
    ...
    for task_key in ("task_a", "task_b", "task_c"):
        visible_signals = _scaled_b_task_signals(task_key, pass_count=pass_count, memory_window=memory_window)
...
def build_b_scale_cases(*, memory_window: int = DEFAULT_MEMORY_WINDOW) -> tuple[BScaleCase, ...]:
    cases: list[BScaleCase] = []
    for difficulty_index, pass_count, description, base_scenario_name, layer_widths in CASE_SCALE_SPECS:
        benchmark_id = f"B{memory_window}S{difficulty_index}"
...
def b_scale_suite_by_id(*, memory_window: int = DEFAULT_MEMORY_WINDOW) -> Dict[str, BScaleCase]:
    return {case.benchmark_id: case for case in build_b_scale_cases(memory_window=memory_window)}
...
def _task_scenario(case: BScaleCase, task_key: str, method_id: str) -> ScenarioSpec:
...
def evaluate_b_scale_suite(
    *,
    ...
    suite = b_scale_suite_by_id(memory_window=memory_window)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Dedicated B-family scaling harness for REAL.")
    parser.add_argument("--memory-window", type=int, default=DEFAULT_MEMORY_WINDOW)
