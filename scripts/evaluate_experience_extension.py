...
def _ordered_signal_events(spec: ScenarioSpec) -> List[Tuple[int, SignalSpec]]:
    events: List[Tuple[int, SignalSpec]] = [(0, signal_spec) for signal_spec in spec.initial_signal_specs]
    for cycle in sorted(dict(spec.signal_schedule_specs or {})):
        events.extend((int(cycle), signal_spec) for signal_spec in dict(spec.signal_schedule_specs or {}).get(cycle, ()))
    return events


def _signal_pass_span(spec: ScenarioSpec) -> int:
    events = _ordered_signal_events(spec)
    if not events:
        return max(1, int(spec.cycles))
    return int(max(cycle for cycle, _ in events)) + 1


def _repeat_signal_scenario(spec: ScenarioSpec, repeat_count: int) -> ScenarioSpec:
    events = _ordered_signal_events(spec)
    if not events:
    ...
    for repeat_index in range(max(int(repeat_count), 1)):
        ...
        repeated_events.extend((cycle + offset, signal_spec) for cycle, signal_spec in events)
    initial_signal_specs = tuple(signal_spec for cycle, signal_spec in repeated_events if cycle == 0)
    ...
    for cycle, signal_spec in repeated_events:
        ...
        signal_schedule_specs[int(cycle)] = signal_schedule_specs[int(cycle)] + (signal_spec,)
    repeated_cycles = int(last_signal_cycle + (max(int(repeat_count), 1) - 1) * pass_span + tail_slack)
    return replace(
        spec,
...
def _run_experience_case(
    *,
    ...
    scenario = _repeat_signal_scenario(base_scenario, repeat_count)
    ...
    pass_span = _signal_pass_span(base_scenario)
