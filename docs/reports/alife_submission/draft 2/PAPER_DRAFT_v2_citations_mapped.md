...
**Biologically-plausible learning.** REAL connects to a long tradition of local learning rules that avoid global credit assignment — Hebbian learning, predictive coding [@rao1999predictive], contrastive Hebbian learning [@movellan1991contrastive], and more recently equilibrium propagation [@scellier2017equilibrium]. REAL differs from this tradition in that it is not attempting to approximate backpropagation with a local rule, but to replace the optimization objective entirely with allostatic coherence.

**Stigmergy and distributed learning.** The substrate-as-memory architecture has structural parallels with ant colony optimization [@dorigo1996antsystem] and stigmergic multi-agent systems [@theraulaz1999stigmergy]. REAL differs in orienting toward generalization and cross-task transfer rather than combinatorial optimization.

**Reservoir computing.** Liquid state machines and echo state networks also use substrate structure for computation [@maass2002realtime; @jaeger2001echo]. Their substrates are random or fixed, not learned through metabolic feedback; transfer is not a design objective; and local decisions are no more interpretable than in standard networks.

**Interpretable machine learning.** Post-hoc methods — SHAP [@lundberg2017shap], LIME [@ribeiro2016lime], integrated gradients [@sundararajan2017integrated] — approximate explanations of trained models through input perturbation or activation analysis. These are correlational by construction. REAL's interpretability is causal: the explanation of a routing decision is the decision record itself, not an approximation derived from perturbing the input space.
